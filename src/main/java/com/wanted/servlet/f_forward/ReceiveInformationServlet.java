...
public class ReceiveInformationServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        *   해당 서블릿은 사용자의 요청이 들어오면 비즈니스 로직을 처리하는
        *   용도의 책임만 갖게 만들고자 한다.
        ...
        * */
        ...
        rd.forward(req, resp); //그대로 전달.

        /* comment
        *    중요!!
        *   forward 는 서로 다른 서블릿과 req, resp 를 공유한다.
        *   따라서 요청을 위임받는 서블릿에서도 정보를 공유할 수 있게 된다.
        *   forward 받은 서블릿의 존재를 클라이언트가 알 필요가 없기 때문에
        *   url 자체는 변경되지 않는다.
        *   forward 방식의 특징은 요청 시 서버로 전송한 데이터가 남아있는 상태로
        *   새로고침(재요청)을 하게 되면 동일한 요청을 반복해서 DB 에 Insert 하는
        *   등의 행위가 발생할 수 있다.
        *   따라서 조회 시에는 forward 방식으로 하지만
        *   Insert 등의 DML 구믄은 forward 방식으로 처리하면 큰일난다.
        * */
    }
}
